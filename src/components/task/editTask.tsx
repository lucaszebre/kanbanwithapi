...
const EditTask = (props: {
  ...
  const task = taskManager[0].boards[currentBoardIndex].columns
  ...
  useEffect(() => {
    const initialColumnErrors = subTasked.map(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (taskManager && taskManager[0].boards[currentBoardIndex].id) {
      updateTask(
        ...
        taskDescription,
        subTasktoDelete,
        subTasked.filter((sub) => sub.add).map((sub) => sub.title),
        subTask,
  ...
  return (
    <div
      className={styles.EditTaskWrapper}
      style={{
    ...
    >
      ...
      >
        <h3
          className={`${styles.EditTaskHeader} ${
            theme === "light" ? styles.light : styles.dark
          }`}
        >
          Edit Task
        ...
        <form
          ...
          onSubmit={async (e) => {
            e.preventDefault();
            const newColumnErrors = subTasked.map(
              (sub) => sub.title.trim() === ""
            );
        ...
        >
          <label
            className={`${styles.EditTaskLabel} ${
              theme === "light" ? styles.light : styles.dark
            }`}
            htmlFor="boardName"
          >
          ...
          />
          {inputError && (
            <div className={styles.ErrorMessage}>Please enter a Task name.</div>
          )}
          ...
          <button
            className={styles.EditTaskAddButton}
            type="button"
          ...
          <button className={styles.EditTaskSaveButton} type="submit">
            Save Changes
          </button>
        </form>
      </div>
