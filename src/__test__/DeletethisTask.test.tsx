...
import DeleteThisTask from '@/components/DeletethisTask';
...
describe('DeleteThisTask component', () => {
  ...
  it('renders the DeleteThisTask component', async () => {
    ...
    const mockUseQuery = jest.fn().mockReturnValue({ data: { boards: [] }, isLoading: false, isError: false });
    ...
    render(
        ...
        <ThemeProvidered>
            <QueryClientProvider client={queryClient}>
                <DeleteThisTask TaskTitle={TaskTitle} TaskId={TaskId} columnId={columnId} />
            </QueryClientProvider>
        </ThemeProvidered>
  ...
  it('calls useDeleteTaskMutation when "Delete" button is clicked', async () => {
    ...
    // Mock the useQuery result
    const mockUseQuery = jest.fn().mockReturnValue({ data: { boards: [{ id: 'board1' }], currentBoardIndex: 0 }, isLoading: false, isError: false });
    require('react-query').useQuery.mockImplementation(mockUseQuery);
  
    // Mock the useMutation function
    const mockMutation = jest.fn();
    require('../utils/useDeleteTaskMutation').default.mockReturnValue({ mutate: mockMutation });
  ...
  it('closes the dialog when "Cancel" button is clicked', async () => {
    ...
    // Mock the useQuery result
    const mockUseQuery = jest.fn().mockReturnValue({ data: { boards: [{id:'ddd'}] }, isLoading: false, isError: false });
    require('react-query').useQuery.mockImplementation(mockUseQuery);
