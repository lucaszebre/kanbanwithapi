import { fetchBoards } from "@/api/board/fetchBoard";
import EditBoard from "@/components/board/editBoard";
import { handleSaveChanges } from "@/components/board/handleSave";
...
describe("EditBoard component", () => {
  ...
  it("allows editing the board name", async () => {
    ...
    fireEvent.change(boardNameInput, {
      target: { value: "Updated Board Name" },
    });
  ...
  it("allows adding a new column", async () => {
    ...
    const columnInputs = await screen.findAllByPlaceholderText(
      "eg Column Name"
    );
  ...
  it("submits the form successfully when valid data is entered", async () => {
    ...
    fireEvent.change(columnInputs[0], {
      target: { value: "Updated Column 1" },
    });
    fireEvent.change(columnInputs[1], {
      target: { value: "Updated Column 2" },
    });
    ...
    expect(handleSaveChanges).toHaveBeenCalledWith(
      [],
      [
        { add: false, id: "1", name: "Updated Column 1", tasks: [] },
        { add: false, id: "2", name: "Updated Column 2", tasks: [] },
      ],
      [],
      "board1",
      "Updated Board Name",
