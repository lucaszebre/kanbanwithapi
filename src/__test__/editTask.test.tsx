...
it('we can fill the form right',async ()=>{
    ...
    render(
        ...
        const TaskInput = (await screen.findByPlaceholderText('task name')) as HTMLInputElement;

        const SubTasksInput = (await screen.findAllByPlaceholderText('eg Column Name')) as HTMLInputElement[] ;
            await waitFor(async () => {
...
it('we can delete subtask',async ()=>{
    ...
    render(
        ...
        const DeleteSubtask = (await screen.findAllByAltText("DeleteTaskIcon")) as HTMLInputElement[] ;
...
it('when click the outside the components , components should not be visible',async ()=>{
    ...
    const OutsideDiv = (container.querySelector('.EditTaskWrapper')) as HTMLInputElement;
    ...
    it('renders the EditTask component with initial data', async () => {
        ...
        const TaskNameInput = (await screen.findByPlaceholderText('task name')) as HTMLInputElement;
        ...
        expect(await screen.findAllByPlaceholderText('eg Column Name')).toHaveLength(3); // Assuming there are no subtasks initially
